...
int main() {
    ...
    load_libs();  // загружаем все .so из libs/

    if (op_count == 0) {
        printf("Нет загруженных операций. Убедитесь, что каталог 'libs' содержит .so файлы.\n");
        return 1;
    }
    ...
    do {
        ...
        Operation* op = &operations[choice - 1];  // ← просто берём по индексу
