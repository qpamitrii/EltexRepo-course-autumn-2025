...
void loadContacts() {
    ...
    while (read(fd, &temp, sizeof(Contact)) == sizeof(Contact)) {
        if (temp.used) { // проверяем флаг — чтобы не читать "удалённые"
            phonebook[contactCount] = temp;
            printf("Загружен контакт: %s %s\n", phonebook[contactCount].surname, phonebook[contactCount].name);
            contactCount++;
    ...
    close(fd);
    printf("Загрузка завершена. Всего контактов: %d\n", contactCount);
}


void saveContacts() {
    printf("\nСохраняем контакты в файл: %s\n", filename);
    ...
    for (int i = 0; i < MAX_CONTACTS; i++) {
        if (phonebook[i].used) {
            if (write(fd, &phonebook[i], sizeof(Contact)) != sizeof(Contact)) {
                perror("Ошибка записи контакта");
                close(fd);
                return;
            }
            printf("Сохранен контакт: %s %s\n", phonebook[i].surname, phonebook[i].name);
        }
    }
